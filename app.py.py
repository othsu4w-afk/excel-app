...
def process_3days_sheet(df_3days_raw):
    ...
    if type_col:
        ...
        cleaned_cats = [clean_text(c) for c in target_cats]
        df_filtered['_clean_type'] = df_filtered[type_col].apply(clean_text)
...
def process_new_cases(df_3days_raw, df_diversity):
    ...
    cleaned_new_types = [clean_text(t) for t in new_case_types]

    df_source = df_3days_raw if df_3days_raw is not None else df_diversity

    if df_source is None or df_source.empty:
        return None, None

    type_col = next(
        (c for c in df_source.columns if '類別' in str(c) or '項目' in str(c) or '來源' in str(c)),
        None,
    )
    ...
    df_source['_clean_type'] = df_source[type_col].apply(clean_text)

    # 1. 圖1：新案來源統計
    df_new_filtered = df_source[df_source['_clean_type'].isin(cleaned_new_types)].copy()

    if df_new_filtered.empty:
        return None, None

    counts = df_new_filtered['_clean_type'].value_counts()
    summary_list = []
    total_new = len(df_new_filtered)

    for original_type in new_case_types:
        c_t = clean_text(original_type)
        cnt = counts.get(c_t, 0)
        pct = f'{round((cnt / total_new) * 100, 1)}%' if total_new > 0 else '0%'
        summary_list.append({
            '新案來源類別': original_type,
            '案數': cnt,
            '比例': pct,
        })
    ...
    # 2. 圖2：新案依「區域」及「個管」交叉統計 (由大到小排序)
    region_col = next(
        (c for c in df_new_filtered.columns if any(k in c for k in ['居住', '區域', '鄉鎮', '縣市', '地址'])),
        None,
    )
    manager_col = next(
        (c for c in df_new_filtered.columns if any(k in c for k in ['個管', 'A個管', '專員', '主管'])),
        None,
    )
